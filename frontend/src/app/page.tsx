...
export default function MarketIntelligenceDashboard() {
  ...
  const [industry, setIndustry] = useState('Fintech & Payment Infrastructure');
  const [competitors, setCompetitors] = useState('Adyen, PayPal, Square, Checkout.com');
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'competitors' | 'swot' | 'strategy' | 'risks'>('overview');
  const [recentReports, setRecentReports] = useState<MarketReport[]>([]);

  // Load initial demo report on first mount
  useEffect(() => {
    async function loadInitial() {
      const initial = await intelligenceApi.quickAnalyze({
        company_name: 'Stripe',
        industry: 'Fintech & Payment Infrastructure',
        target_competitors: ['Adyen', 'PayPal', 'Square', 'Checkout.com'],
      });
    ...
    <main style={{ minHeight: '100vh', padding: '32px 24px', maxWidth: '1440px', margin: '0 auto' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
              Autonomous Market & Competitor Intelligence Platform
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span className="badge badge-primary">v1.0.0 Enterprise</span>
        </div>
      </header>
      ...
      >
        <div className="glass-panel" style={{ padding: '20px' }}>
          ...
          <span style={{ fontSize: '0.75rem', color: 'var(--secondary-accent)' }}>
            {activeReport?.market_overview?.cagr ? `Growing at ${activeReport.market_overview.cagr}` : 'Market Research'}
          </span>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '20px' }}>
          ...
          <p style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--success)' }}>
            {activeReport?.market_overview?.tam || '$50B+'}
          </p>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            SAM: {activeReport?.market_overview?.sam || '$15B+'}
          </span>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '20px' }}>
          ...
          <p style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--warning)' }}>
            {activeReport?.strategic_recommendations?.length || 3} Strategies
          </p>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            ...
            <form onSubmit={handleStartResearch} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              >
                {isLoading ? 'Agent Synthesizing...' : '⚡ Generate Dossier'}
              </button>
            </form>
          </div>
        ...
        <section className="glass-panel" style={{ padding: '32px' }}>
          {activeReport ? (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                  ...
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Generated by Alkame Autonomous Agent • Confidence Score: 95%
                  </p>
                </div>
              </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '28px' }}>
                    <div className="glass-panel" style={{ padding: '18px' }}>
                      ...
                      <p style={{ fontSize: '1.35rem', fontWeight: '700', color: 'var(--success)' }}>
                        {activeReport.market_overview.tam || '$45B+'}
                      </p>
                    ...
                    <div className="glass-panel" style={{ padding: '18px' }}>
                      ...
                      <p style={{ fontSize: '1.35rem', fontWeight: '700', color: 'var(--primary-accent)' }}>
                        {activeReport.market_overview.sam || '$14B+'}
                      </p>
                    ...
                    <div className="glass-panel" style={{ padding: '18px' }}>
                      ...
                      <p style={{ fontSize: '1.35rem', fontWeight: '700', color: 'var(--warning)' }}>
                        {activeReport.market_overview.cagr || '18.4%'}
                      </p>
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.88rem' }}>Enter a company name on the left and click Generate Dossier to launch the agent.</p>
            </div>
          )}
        </section>
      </div>
